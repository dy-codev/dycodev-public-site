...
function postCarousel() {
  return {
      ...
      async loadPosts() {
          ...
          this.slides = posts.map(p => {

              // Categories
              const categories = (p._embedded?.["wp:term"]?.[0] || [])
                  .map(c => `<a href="${c.link}" class="hover:underline">${c.name}</a>`)
                  .join(", ");
              ...
              let cleanTitle = decodeHtml(p.title.rendered)
                .replace(/<\/?[^>]+(>|$)/g, "")  // buang tag
                .replace(/\s+/g, " ")            // rapikan spasi
                .trim();
                
              const title = cleanTitle.length > 55
                  ? cleanTitle.slice(0, 55) + "…"
                  : cleanTitle;

              // Excerpt
              const tmp = document.createElement("div");
              tmp.innerHTML = p.excerpt.rendered;
              let excerpt = tmp.textContent.trim();
